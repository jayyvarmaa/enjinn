...
void enjinn::ContainersWindow::init(enjinn::enjinnImgui::ImGuiIdsManager &imguiIdsManager)
{
	imguiIds = imguiIdsManager.getImguiIds(10);
}

void enjinn::ContainersWindow::update(enjinn::LogManager &logManager, bool &open, enjinn::LoadedDll &loadedDll,
	enjinn::ContainerManager &containerManager, enjinn::enjinnImgui::ImGuiIdsManager &imguiIdsManager, enjinn::ConsoleWindow *consoleWindow)
{
...

	{
		...
		if (ImGui::BeginTabBar("##Tabs", ImGuiTabBarFlags_Reorderable))
		{
				...
				ImGui::BeginGroup();
				{
					if (itemCurrentAvailableCOntainers < loadedDll.containerInfo.size())
					{
						...
						#pragma region total memory requirement
							...
							ImGui::SameLine();
							enjinn::enjinnImgui::displayMemorySizeValue(totalMemory);

							ImGui::Text("Total Heap requirement: ");
							ImGui::SameLine();
							enjinn::enjinnImgui::displayMemorySizeValue(totalHeapMemory);
						#pragma endregion
						...
						ImGui::SameLine();
						enjinn::enjinnImgui::displayMemorySizeValue(c.containerStructBaseSize);

						ImGui::Text("Default Heap Memory requirement: ");
						ImGui::SameLine();
						enjinn::enjinnImgui::displayMemorySizeValue(c.containerStaticInfo.defaultHeapMemorySize);

						ImGui::Text("Other Heap Memory Allocators count: ");
						ImGui::SameLine();
						enjinn::enjinnImgui::displayMemorySizeValue(c.containerStaticInfo.bonusAllocators.size());
						...
						if (!c.containerStaticInfo.bonusAllocators.empty())
						{
								...
								{0, 100}, true, ImGuiWindowFlags_AlwaysVerticalScrollbar))
							{
								...
								for (auto i : c.containerStaticInfo.bonusAllocators)
								{
									enjinn::enjinnImgui::displayMemorySizeValue(i);
								}
								...
								&& ImGui::Button(ICON_FK_PLAY " Launch a default configuration"))
							{
								if (createAtSpecificMemoryRegion)
								{
									containerManager.createContainer(selectedContainerToLaunch, loadedDll,
										logManager, imguiIdsManager, consoleWindow, std::string(), enjinn::TB(1));
								}
								else
								{
...

							{
								...
								auto snapshots = enjinn::getAvailableSnapshotsAnyMemoryPosition(c);
									...
									&& ImGui::Button(ICON_FK_PICTURE_O " Launch a snapshot"))
								{
									...
									if (memPos == nullptr)
									{
										logManager.log("Failes to get snapshot info", enjinn::logError);
									}
									else
									{
			...
			if (ImGui::BeginTabItem(ICON_FK_MICROCHIP " Running containers"))
			{
				...
				//left
				std::vector<enjinn::containerId_t> containerIds;
				std::vector<std::string> containerNames;
				...
				ImGui::BeginGroup();
				{
					...
					#pragma region buttons
						...
						//calculate cursor pos for 3 buttons
						{
							...
							enjinn::enjinnImgui::alignForWidth(width);
						}

						if (c.flags.status == enjinn::RuntimeContainer::FLAGS::STATUS_RUNNING)
						{
							if (ImGui::Button(ICON_FK_PAUSE))
							{
								c.flags.status = enjinn::RuntimeContainer::FLAGS::STATUS_PAUSE;
							}
						...
						else if (c.flags.status == enjinn::RuntimeContainer::FLAGS::STATUS_PAUSE)
						{
							if (ImGui::Button(ICON_FK_PLAY))
							{
								c.flags.status = enjinn::RuntimeContainer::FLAGS::STATUS_RUNNING;
							}
						...
						else if (c.flags.status == enjinn::RuntimeContainer::FLAGS::STATUS_BEING_RECORDED)
						{
							ImGui::BeginDisabled();
							if (ImGui::Button(ICON_FK_PAUSE))
							{
						...
						else if (c.flags.status == enjinn::RuntimeContainer::FLAGS::STATUS_BEING_PLAYBACK)
						{
							...
							if (ImGui::BeginTabBar("##Tabs for play and record", ImGuiTabBarFlags_Reorderable))
							{
								if (ImGui::BeginTabItem(ICON_FK_CAMERA " Snapshot"))
								{
									//snapshot button
									{
										...
										{
											if (enjinn::isFileNameValid(snapshotName, sizeof(snapshotName)))
											{
												if (!containerManager.makeSnapshot(containerIds[itemCurrentCreatedContainers], logManager, snapshotName))
												{
													logManager.log("Coultn't make snapshot", enjinn::logError);
												}
												else
												{
											...
											{
												logManager.log("File name invalid", enjinn::logError);
											}
								...
								#pragma region snapshots
									{
										...
										auto snapshots = enjinn::getAvailableSnapshots(
										...
										if (ImGui::Button(ICON_FK_PLAY "##play snapshot"))
										{
											...
											{
												logManager.log("Failed to assign snapshot", enjinn::logError);
											}
										}
								...
								if (ImGui::BeginTabItem(ICON_FK_VIDEO_CAMERA " Record"))
								{
									//recording
									{
										if (c.flags.status == enjinn::RuntimeContainer::FLAGS::STATUS_RUNNING)
										{
											...
											{
												if (enjinn::isFileNameValid(recordingName, sizeof(recordingName)))
												{
										...
										ImGui::BeginDisabled(c.flags.status == enjinn::RuntimeContainer::FLAGS::STATUS_BEING_RECORDED);
										ImGui::SameLine();
								...
								#pragma region recordings
									...
									auto recordings = enjinn::getAvailableRecordings(
										containerManager.runningContainers[containerIds[itemCurrentCreatedContainers]]);
									...
									if (recordings.size() == 0 || currentSelectedRecording >= recordings.size()
										|| c.flags.status == enjinn::RuntimeContainer::FLAGS::STATUS_BEING_RECORDED)
									{
										ImGui::BeginDisabled(true);
									}
									else
									...
									if (ImGui::Button(ICON_FK_PLAY "##play recording"))
									{
										...
										{
											logManager.log("Failed to assign recording", enjinn::logError);
										}
									}

									ImGui::EndDisabled();
														
									if (c.flags.status == enjinn::RuntimeContainer::FLAGS::STATUS_BEING_RECORDED)
									{
										if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled))
										{
