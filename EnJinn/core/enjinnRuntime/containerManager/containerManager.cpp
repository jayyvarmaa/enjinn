...
enjinn::containerId_t enjinn::ContainerManager::createContainer(std::string containerName, 
	enjinn::LoadedDll &loadedDll, enjinn::LogManager &logManager, 
	enjinn::enjinnImgui::ImGuiIdsManager &imguiIDsManager, ConsoleWindow *consoleWindow, std::string &cmd, size_t memoryPos)
{
	...
	logManager.log(("Couldn't create container, couldn't find the name: " + containerName).c_str(), enjinn::logError);

	return 0;
}

bool enjinn::ContainerManager::setSnapshotToContainer(enjinn::containerId_t containerId, const char *snapshotName, enjinn::LogManager &logManager
	, enjinn::enjinnImgui::ImGuiIdsManager &imguiIdManager)
{
	...
	std::string file = ENJINN_ENGINE_RESOURCES_PATH;
	file += snapshotName;
	file += ".snapshot";

	auto s = enjinn::getFileSize(file.c_str());

	if (s != (c->second.totalSize + sizeof(enjinn::RuntimeContainer)) ) 
	{
...
bool enjinn::ContainerManager::setRecordingToContainer(enjinn::containerId_t containerId, const char *recordingName, 
	enjinn::LogManager &logManager, enjinn::enjinnImgui::ImGuiIdsManager &imguiIdManager)
{
	...
	c->second.flags.status = enjinn::RuntimeContainer::FLAGS::STATUS_BEING_PLAYBACK;
	c->second.flags.frameNumber = 0;
	enjinn::strlcpy(c->second.flags.recordingName, recordingName, sizeof(c->second.flags.recordingName));

}

//todo mabe use regions further appart in production
void* enjinn::ContainerManager::allocateContainerMemory(enjinn::RuntimeContainer &container,
	enjinn::ContainerInformation containerInformation, void *memPos)
{
...
void enjinn::ContainerManager::allocateContainerMemoryAtBuffer(enjinn::RuntimeContainer &container,
	enjinn::ContainerInformation containerInformation, void *buffer)
{
	...
	enjinn::align64(currentMemoryAdress);
...
void enjinn::ContainerManager::freeContainerStuff(enjinn::RuntimeContainer &container)
{
...
enjinn::containerId_t enjinn::ContainerManager::createContainer
(enjinn::ContainerInformation containerInformation,
	enjinn::LoadedDll &loadedDll, enjinn::LogManager &logManager, enjinn::enjinnImgui::ImGuiIdsManager &imguiIDsManager,
...
{	
	...
	//{
	//	logManager.log((std::string("Container id already exists: #") + std::to_string(id)).c_str(), enjinn::logError);
	//	return false;
	//}

	//todo a create and destruct wrapper

	enjinn::RuntimeContainer container = {};
	enjinn::strlcpy(container.baseContainerName, containerInformation.containerName,
	...
	if (!allocateContainerMemory(container, containerInformation, (void*)memoryPos))
	{
		logManager.log((std::string("Couldn't allocate memory for constructing container: #") 
			+ std::to_string(id)).c_str(), enjinn::logError);
		return 0;
	}

	//create imgui fbo just for developement mode
#ifndef ENJINN_PRODUCTION
	...
	if (!loadedDll.constructRuntimeContainer(container, containerInformation.containerName.c_str()))
	{
		...
		logManager.log((std::string("Couldn't construct container: #") + std::to_string(id)).c_str(), enjinn::logError);
...
#pragma endregion
	...
	if (!rezult)
	{
		logManager.log((std::string("Couldn't create container because it returned 0")
			+ container.baseContainerName + " #" + std::to_string(id)).c_str(), enjinn::logWarning);
...
void enjinn::ContainerManager::init()
{
}

void enjinn::ContainerManager::update(enjinn::LoadedDll &loadedDll, enjinn::EnJinnWindow &window,
	enjinn::LogManager &logs, enjinn::enjinnImgui::ImGuiIdsManager &imguiIdManager, enjinn::ConsoleWindow *consoleWindow)
{
	ENJINN_DEVELOPMENT_ONLY_ASSERT(loadedDll.dllHand != 0, "dll not loaded when trying to update containers");
...
#pragma region running containers
	for (auto &c : runningContainers)
	{
		...
		if (c.second.flags.status == enjinn::RuntimeContainer::FLAGS::STATUS_RUNNING
			||
			c.second.flags.status == enjinn::RuntimeContainer::FLAGS::STATUS_BEING_RECORDED
			||
			c.second.flags.status == enjinn::RuntimeContainer::FLAGS::STATUS_BEING_PLAYBACK
			)
		{
			
			ENJINN_DEVELOPMENT_ONLY_ASSERT(
			...
			if (c.second.flags.status == enjinn::RuntimeContainer::FLAGS::STATUS_BEING_RECORDED)
			{
				if (!makeRecordingStep(c.first, logs, windowInput))
				{

					c.second.flags.status = enjinn::RuntimeContainer::FLAGS::STATUS_RUNNING;
			...
			if (c.second.flags.status == enjinn::RuntimeContainer::FLAGS::STATUS_BEING_PLAYBACK)
			{
				enjinn::Input readInput = {};

				std::string fileName = c.second.flags.recordingName;
				fileName += ".recording";
				fileName = ENJINN_ENGINE_RESOURCES_PATH + fileName;

				auto s = enjinn::getFileSize(fileName.c_str());
				if (c.second.flags.frameNumber * sizeof(enjinn::Input) >= s && s != 0)
				{
				...
				else if (s % sizeof(enjinn::Input) != 0)
				{
				...
				if (!enjinn::readEntireFile(fileName.c_str(), &readInput, sizeof(enjinn::Input), sizeof(enjinn::Input) * c.second.flags.frameNumber))
				{
			...
			auto callUpdate = [&](enjinn::WindowState &windowState) -> bool
			{
			...
			#pragma endregion
				...
				// Fix: Unlock cursor on ESC
				// Fix: Unlock cursor on ESC
				if (windowInput.hasFocus && ImGui::IsKeyPressed(ImGuiKey_Escape))
				{
					glfwSetInputMode(window.context.wind, GLFW_CURSOR, GLFW_CURSOR_NORMAL);
					ImGui::SetWindowFocus(nullptr); // Unfocus the ImGui window
					windowInput.hasFocus = false;   // Suppress focus for this frame
					c.second.lastFrameFocus = false; // Reset internal tracking
					logs.log("Cursor unlocked (ESC pressed)");
				}
...
bool enjinn::ContainerManager::reloadDll(enjinn::LoadedDll &loadedDll, enjinn::EnJinnWindow &window, enjinn::LogManager &logs)
{
	...
	{
		logs.log("Couldn't reloaded dll", enjinn::logWarning);
		return 0;
	}
	//todo pospone dll reloading and make this timer shorter

	std::unordered_map<std::string, enjinn::ContainerInformation> containerNames;
	for (auto &c : loadedDll.containerInfo)
	{
		containerNames[c.containerName] = c;
	}

	std::unordered_map<std::string, enjinn::ContainerInformation> oldContainerNames;
	...
	{
		std::vector<enjinn::containerId_t> containersToClean;
	...
	//realocate pointers
	{
		...
		for (auto &i : runningContainers)
		{
			...
			{
				enjinn::RuntimeContainer container = {};
				enjinn::strlcpy(container.baseContainerName, i.second.baseContainerName,
					sizeof(container.baseContainerName));

				enjinn::ContainerInformation info;
				...
				{
					logs.log("Internal error 1 (couldn't allocate container memory)", enjinn::logError);
				}
				else
				{
					loadedDll.bindAllocatorDllRealm(&container.allocator);
					...
					{
						loadedDll.resetAllocatorDllRealm();
						logs.log("Internal error 2", enjinn::logError);
						freeContainerStuff(container);
						loadedDll.resetAllocatorDllRealm();
					}
					else
...
bool enjinn::ContainerManager::destroyContainer(containerId_t id, enjinn::LoadedDll &loadedDll,
	enjinn::LogManager &logManager)
{
	ENJINN_DEVELOPMENT_ONLY_ASSERT(loadedDll.dllHand != 0, "dll not loaded when trying to destroy container");
...
// 
//
bool enjinn::ContainerManager::makeSnapshot(containerId_t id, enjinn::LogManager &logManager, const char *fileName)
{
	auto c = runningContainers.find(id);
	if (c == runningContainers.end())
	{
	...
	std::string filePath = ENJINN_ENGINE_RESOURCES_PATH;
	filePath += fileName;

	filePath += ".snapshot";
		

	if(!enjinn::writeEntireFile(filePath.c_str(), &c->second, sizeof(c->second)))
	{
	...
	if (!enjinn::appendToFile(filePath.c_str(), 
		c->second.getBaseAdress(), c->second.totalSize))
	{
		enjinn::deleteFile(filePath.c_str());
		logManager.log(("Couldn't write to file for making snapshot: " + filePath).c_str(),
			enjinn::logError);
		return false;
	}
...
bool enjinn::ContainerManager::startRecordingContainer(containerId_t id, enjinn::LogManager &logManager, const char *fileName)
{
	auto c = runningContainers.find(id);
	if (c == runningContainers.end())
	{
	...
	c->second.flags.status = enjinn::RuntimeContainer::FLAGS::STATUS_BEING_RECORDED;
	enjinn::strlcpy(c->second.flags.recordingName, filePath, sizeof(c->second.flags.recordingName));



}

bool enjinn::ContainerManager::stopRecordingContainer(containerId_t id, enjinn::LogManager &logManager)
{
	auto c = runningContainers.find(id);
	if (c == runningContainers.end())
	{
...
bool enjinn::ContainerManager::makeRecordingStep(containerId_t id, enjinn::LogManager &logManager,
	enjinn::Input &input)
{
	auto c = runningContainers.find(id);
	if (c == runningContainers.end())
	...
	if (!enjinn::appendToFile(c->second.flags.recordingName, &input, sizeof(input)))
	{
...
bool enjinn::ContainerManager::forceTerminateContainer(containerId_t id, enjinn::LoadedDll &loadedDll, enjinn::LogManager &logManager)
{
	ENJINN_DEVELOPMENT_ONLY_ASSERT(loadedDll.dllHand != 0, "dll not loaded when trying to destroy container");
...
void enjinn::ContainerManager::destroyAllContainers(enjinn::LoadedDll &loadedDll,
	enjinn::LogManager &logManager)
{
	std::vector < enjinn::containerId_t> containersId;
...
void *enjinn::ContainerManager::allocateOSMemory(size_t size, void *baseAdress)
{
	ENJINN_PERMA_ASSERT(baseAdress == nullptr, "can't allocate fixed memory in production");
	return malloc(size);
}

void enjinn::ContainerManager::deallocateOSMemory(void *baseAdress)
{
	free(baseAdress);
}
...
std::vector<std::string> enjinn::getAvailableSnapshots(enjinn::RuntimeContainer &info)
{
	std::vector<std::string> files;

	auto curDir = std::filesystem::directory_iterator(ENJINN_ENGINE_RESOURCES_PATH);
	...
	for (const auto &iter : curDir)
	{
		...
		{
			if (enjinn::checkIfSnapshotIsCompatible(info, iter.path().stem().string().c_str()))
			{
...
std::vector<std::string> enjinn::getAvailableRecordings(enjinn::RuntimeContainer &info)
{
...
std::vector<std::string> enjinn::getAvailableSnapshotsAnyMemoryPosition(enjinn::RuntimeContainer &info)
{
	std::vector<std::string> files;
	...
	for (const auto &iter : curDir)
	{
		...
		{
			if (enjinn::checkIfSnapshotIsCompatibleAnyMemoryPosition(info, iter.path().stem().string().c_str()))
			{
				files.push_back(iter.path().stem().string());
			}
...
std::vector<std::string> enjinn::getAvailableRecordingAnyMemoryPosition(enjinn::RuntimeContainer &info)
{
...
std::vector<std::string> enjinn::getAvailableSnapshotsAnyMemoryPosition(enjinn::ContainerInformation &info)
{
	std::vector<std::string> files;
...
std::vector<std::string> enjinn::getAvailableRecordingsAnyMemoryPosition(enjinn::ContainerInformation &info)
{
	auto snapshots = getAvailableSnapshotsAnyMemoryPosition(info);
...
bool enjinn::checkIfSnapshotIsCompatible(enjinn::RuntimeContainer &info, const char *snapshotName)
{
	...
	enjinn::RuntimeContainer loadedInfo = {};

	auto s = enjinn::readEntireFile(file.c_str(), &loadedInfo, sizeof(loadedInfo));
...
bool enjinn::checkIfSnapshotIsCompatibleAnyMemoryPosition(enjinn::RuntimeContainer &info, const char *snapshotName)
{
	std::string file = ENJINN_ENGINE_RESOURCES_PATH;
	file += snapshotName;
	file += ".snapshot";
...
bool enjinn::checkIfSnapshotIsCompatibleAnyMemoryPosition(enjinn::ContainerInformation &info, const char *snapshotName)
{
	std::string file = ENJINN_ENGINE_RESOURCES_PATH;
	file += snapshotName;
	file += ".snapshot";
...
void *enjinn::getSnapshotMemoryPosition(const char *snapshotName)
{
	std::string file = ENJINN_ENGINE_RESOURCES_PATH;
	file += snapshotName;
	file += ".snapshot";
