...
#define GAMEPLAYSTART(x) void x(enjinn::EnJinnContext &enjinnContext, enjinn::LogManager &logs)
typedef GAMEPLAYSTART(gameplayStart_t);
#undef GAMEPLAYSTART

#define GAMEPLAYRELOAD(x) void x(enjinn::EnJinnContext &enjinnContext, enjinn::LogManager &logs)
typedef GAMEPLAYRELOAD(gameplayReload_t);
#undef GAMEPLAYRELOAD

#define GETCONTAINERSINFO(x) void x(std::vector<enjinn::ContainerInformation> &info)
typedef GETCONTAINERSINFO(getContainersInfo_t);
#undef GETCONTAINERSINFO

#define CONSTRUCTCONTAINER(x) bool x(Container **c, enjinn::memory::MemoryArena *arena, const char *name);
typedef CONSTRUCTCONTAINER(constructContainer_t);
#undef CONSTRUCTCONTAINER

#define DESTRUCTCONTAINER(x) void x(Container **c, enjinn::memory::MemoryArena *arena);
typedef DESTRUCTCONTAINER(destructContainer_t);
#undef DESTRUCTCONTAINER

#define BINDALLOCATOR(x) void x(enjinn::memory::FreeListAllocator *arena);
...
struct LoadedDll
{
	...
	bool loadDll(int id, enjinn::LogManager &logs);

	bool tryToloadDllUntillPossible(int id, enjinn::LogManager &logs, std::chrono::duration<long long> timeout = 
		std::chrono::seconds(0));

	void unloadDll();

	//no need to call since it is called in load dll function
	void getContainerInfoAndCheck(enjinn::LogManager &logs);

	bool shouldReloadDll();

	void reloadContainerExtensionsSupport();

	std::vector<enjinn::ContainerInformation> containerInfo;

	std::unordered_map<std::string, std::vector<std::string>> containerExtensionsSupport;

	bool constructRuntimeContainer(enjinn::RuntimeContainer &c, const char *name);

	void bindAllocatorDllRealm(enjinn::memory::FreeListAllocator *allocator);
