...
::gl2d::Texture enjinn::gl2d::loadTextureWithPixelPadding(const char *path, RequestedContainerInfo &info, int blockSize,
...
::gl2d::Texture enjinn::gl2d::loadTexture(const char *path, RequestedContainerInfo &info, bool pixelated, bool useMipMaps)
{
	::gl2d::Texture t = {};
	size_t s = 0;
	if (info.getFileSizeBinary(path, s))
...
::gl2d::Font enjinn::gl2d::loadFont(const char *path, RequestedContainerInfo &info)
{
	::gl2d::Font f = {};
	size_t s = 0;
	if (info.getFileSizeBinary(path, s))
	{
		...
		else
		{
			...
			//todo
			//info.log((std::string("error loading font, parsing file: ") + path).c_str(), enjinn::logError);
	...
	else
	{
		...
		//todo
		//info.log((std::string("error loading font, openning file: ") + path).c_str(), enjinn::logError);
		return {};
	}
...
void enjinn::gl2d::cameraController(::gl2d::Camera &c, Input &input, float speed, float zoomSpeed)
{
