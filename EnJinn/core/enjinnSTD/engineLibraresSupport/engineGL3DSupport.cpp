...
void enjinn::gl3d::generalSettingsWindow(int imguiId, ::gl3d::Renderer3D &renderer)
{
...
void enjinn::gl3d::fxaaSettingsWindow(int imguiId, ::gl3d::Renderer3D &renderer)
{
	ImGui::PushID(imguiId);
...
void enjinn::gl3d::ssaoSettingsWindow(int imguiId, ::gl3d::Renderer3D &renderer)
{
	ImGui::PushID(imguiId);
...
void enjinn::gl3d::ssrSettingsWindow(int imguiId, ::gl3d::Renderer3D &renderer)
{
	ImGui::PushID(imguiId);
...
void enjinn::gl3d::bloomSettingsWindow(int imguiId, ::gl3d::Renderer3D &renderer)
{
	ImGui::PushID(imguiId);
...
void enjinn::gl3d::chromaticAberationSettingsWindow(int imguiId, ::gl3d::Renderer3D &renderer)
{
	ImGui::PushID(imguiId);
...
void enjinn::gl3d::lightEditorSettingsWindow(int imguiId, ::gl3d::Renderer3D &renderer)
{
	ImGui::PushID(imguiId);
...
void enjinn::gl3d::fpsInput(::gl3d::Renderer3D &renderer, enjinn::Input &input, float speed, glm::dvec2 &lastMousePos,
	RequestedContainerInfo &requestedInfo, glm::ivec2 windowSize)
{
	...
	if (input.buttons[enjinn::Button::Q].held())
	{
		dir.y -= speed * input.deltaTime;
	}
	if (input.buttons[enjinn::Button::E].held())
	{
		dir.y += speed * input.deltaTime;
	}
...
bool enjinn::gl3d::loadSettingsFromFileName(::gl3d::Renderer3D &renderer, std::string file, RequestedContainerInfo &info)
{
	std::string data;
...
void enjinn::gl3d::General3DEditor::loadFromFile(::gl3d::Renderer3D &renderer, std::string file, RequestedContainerInfo &info)
{
	std::string data;
	...
	if (info.readEntireFile(file.c_str(), data))
	{
		...
		enjinn::strlcpy(settingsFileSelector.file, file, sizeof(settingsFileSelector.file));
	}
}

void enjinn::gl3d::General3DEditor::update(int imguiId, ::gl3d::Renderer3D &renderer,
	enjinn::Input &input, float moveSpeed, RequestedContainerInfo &info, glm::ivec2 windowSize)
{
	ImGui::PushID(imguiId);
	if (ImGui::Begin("General3DEditor"))
	{
		...
		{
			enjinn::gl3d::generalSettingsWindow(imguiId, renderer);
		}
		...
		if (ImGui::CollapsingHeader("Sky box", ImGuiTreeNodeFlags_Framed | ImGuiTreeNodeFlags_FramePadding))
		{
			...
			if (skyBoxFileSelector.run(imguiId))
			{
				...
				if (ext == ".hdr") //todo enjinn to lower on strings
				{
	...
	enjinn::gl3d::fpsInput(renderer, input, moveSpeed, lastMousePos, info, windowSize);


}

void enjinn::gl3d::General3DEditor::saveToFile(::gl3d::Renderer3D &renderer, RequestedContainerInfo &info)
{
