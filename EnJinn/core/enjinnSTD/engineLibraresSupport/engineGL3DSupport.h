...
namespace enjinn
{
	namespace gl3d
	{
		...
		void fpsInput(::gl3d::Renderer3D &renderer, enjinn::Input &input, float moveSpeed, glm::dvec2 &lastMousePos,
		...
		struct General3DEditor
		{
			General3DEditor() 
			{
				skyBoxFileSelector.setInfo("Sellect skyBox", ENJINN_RESOURCES_PATH, {".hdr", ".png"});
				settingsFileSelector.setInfo("Gl3D file", ENJINN_RESOURCES_PATH, {".gl3d"});
			};

			void loadFromFile(::gl3d::Renderer3D &renderer, std::string file, RequestedContainerInfo &info);

			void update(int imguiId, ::gl3d::Renderer3D &renderer, enjinn::Input &input, float moveSpeed
			...
			enjinn::enjinnImgui::FileSelector skyBoxFileSelector;

			enjinn::enjinnImgui::FileSelector settingsFileSelector;
