...
namespace memory
{
	...
	void FreeListAllocator::init(void *baseMemory, size_t memorySize)
		{
			...
			ENJINN_PERMA_ASSERT(memorySize >= 100, "Memory size must be at least 100 bytes");
	...
	void *FreeListAllocator::allocate(size_t size)
		{
			...
			ENJINN_PERMA_ASSERT(baseMemory, "Allocator not initialized"); //err allocator not initialized
			...
			while (true)
			{
					...
					else //this is not the last free block 
					{
						...
						if (last == nullptr || last >= this->end) // this is the first free block but not the last 
						{
							...
							if (currentSize - aligned8Size < 24)
							{
								...
								{
									//heap corrupted
									ENJINN_PERMA_ASSERT(0, "heap corupted");
								}
				...
				else
				{
					if (current->next == nullptr || current->next >= this->end)
					{
						...
						{
							ENJINN_PERMA_ASSERT(0, "Allocator out of memory");
						}
			...
			ENJINN_PERMA_ASSERT(0, "unreachable code");
			return nullptr;
		}
	...
	void FreeListAllocator::free(void *mem)
		...
		#pragma region check validity
			...
			ENJINN_PERMA_ASSERT(allocatedBLockHeader->guard == GUARD_VALUE, "invalid free or double free"); //invalid free or double free
			...
			else
			{
				...
				while (true)
				{
					...
					if (current == nullptr || current >= this->end)
					{
						...
						ENJINN_PERMA_ASSERT(0, "heap corupted or an invalid free");
