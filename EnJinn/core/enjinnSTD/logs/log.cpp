...
void enjinn::LogManager::init(std::string name)
{
...
void enjinn::LogManager::log(const char *l, int type)
{
	...
	#if !ENJINN_REMOVE_LOGS_TO_FILE_IN_PRODUCTION
	logToFile(l, type);
	#endif

	#if !ENJINN_REMOVE_LOGS_TO_NOTIFICATIONS_IN_PRODUCTION
	logToPushNotification(l, type);
	#endif
...
std::stringstream formatLog(const char *l, int type)
{
	...
	if (type == enjinn::logWarning)
	{
		s << "[warning]";
	}
	else if (type == enjinn::logError)
	{
		s << "[error]";
	}
...
void enjinn::LogManager::logToFile(const char *l, int type)
{
	...
	enjinn::logToFile(name.c_str(), l, type);
}

void enjinn::LogManager::logInternally(const char *l, int type)
{
...
void enjinn::LogManager::logToPushNotification(const char *l, int type)
{
...
void enjinn::logToFile(const char *fileName, const char *l, int type)
{
