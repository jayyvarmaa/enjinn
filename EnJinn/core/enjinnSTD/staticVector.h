...
namespace enjinn
{
	...
	struct StaticVector
	{
		...
		{
			ENJINN_PERMA_ASSERT(index < size_, "buffer overflow on acces");
			return (beg_)[index];
		}
		...
		{
			ENJINN_PERMA_ASSERT(size_ < MAX_SIZE, "exceded max size in push back");
			beg_[size_] = el;
			size_++;
		}
		...
		void pop_back()
		{
			ENJINN_PERMA_ASSERT(size_ > 0, "buffer underflow on pop back");
			size_--;
		}
