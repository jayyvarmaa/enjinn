...
#include <enjinnGL/frameBuffer.h>
...
{
	enjinn::StaticString<257> containerName = {};
	enjinn::StaticString<257> cmdArgs = {};
};
...
{
	enjinn::memory::FreeListAllocator *mainAllocator = {};
	enjinn::StaticVector<enjinn::memory::FreeListAllocator, MaxAllocatorsCount> *bonusAllocators = {};

	//readEntireFile_t *readEntireFilePointer = {};
	//getFileSize_t *getFileSizePointer = {};

	enjinn::GL::EnJinnFramebuffer requestedFBO = {};

	int requestedImguiIds = 0;
	int imguiTotalRequestedIds = 0;
	int pushImguiIdForMe = 0;

	enjinn::ConsoleWindow *consoleWindow = nullptr;

	enjinn::LogManager *logManager = nullptr;
	...
	void setFpsCursor()
	{
		ENJINN_DEVELOPMENT_ONLY_ASSERT(internal.setInputModeFunc, "missing setInputModeFunc func");
	...
	{
		ENJINN_DEVELOPMENT_ONLY_ASSERT(internal.containersToCreate, "missing containersToCreate pointer");
	...
	{
		//ENJINN_DEVELOPMENT_ONLY_ASSERT(readEntireFilePointer, "read entire file pointer not assigned");
		bool success = true;
	...
	{
		//ENJINN_DEVELOPMENT_ONLY_ASSERT(getFileSizePointer, "get file size pointer not assigned");
...
struct ContainerStaticInfo
{
	...
	enjinn::StaticVector<size_t, MaxAllocatorsCount> bonusAllocators = {};

	//add file extensions here so that the engine knows that your container can open them.
	enjinn::StaticVector<enjinn::StaticString<16>, 16> extensionsSuported = {};
...
struct Container
{
	...
	virtual bool create(RequestedContainerInfo &requestedInfo, enjinn::StaticString<256> commandLineArgument) = 0;

	virtual bool update(
		enjinn::Input input, 
		enjinn::WindowState windowState,
