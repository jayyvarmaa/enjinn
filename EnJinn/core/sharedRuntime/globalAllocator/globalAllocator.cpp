...
enjinn::memory::FreeListAllocator *currentCustomAllocator = {};
...
namespace memory
{
	...
	{
		ENJINN_PERMA_ASSERT(!pushed, "Can't edit allocators while you pushed custom allocators to standard");
	...
	{
		pushed--;
		ENJINN_PERMA_ASSERT(pushed>=0, "pop underflow on popCustomAllocatorsToStandard");
		GlobalAllocateFunction = CustomAllocator;
		GlobalFree = CustomFree;
	}
}
