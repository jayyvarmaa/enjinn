...
#define ENJINN_NOT_IMPLEMENT


void enjinn::PushNotificationManager::init()
{}

void enjinn::PushNotificationManager::update(bool &open)
{}

void enjinn::PushNotificationManager::pushNotification(const char *content, int logType)
{}


#endif

#ifndef ENJINN_NOT_IMPLEMENT
...
void enjinn::PushNotificationManager::update(bool &open)
{
	...
	if (ImGui::Begin("Notification content", &open, window_flags))
	{
		...
		for (auto i = 0; i < notificationQue.size(); i++)
		{
			...
			if (notificationQue[i].notificationType == enjinn::logError)
			{
				enjinn::enjinnImgui::addErrorSymbol();
				ImGui::SameLine();
			}

			if (notificationQue[i].notificationType == enjinn::logWarning)
			{
				enjinn::enjinnImgui::addWarningSymbol();
				ImGui::SameLine();
			}
