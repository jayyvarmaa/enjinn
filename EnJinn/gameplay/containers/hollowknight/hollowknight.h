...
struct Holloknight: public Container
{
	...
	bool create(RequestedContainerInfo &requestedInfo, enjinn::StaticString<256> commandLineArgument)
	{
		...
		if (!requestedInfo.readEntireFileBinary(ENJINN_RESOURCES_PATH "hollowknight/inputMetrics.bin",
		...
		{
			assets[i]
				= enjinn::gl2d::loadTexture
				((std::string(ENJINN_RESOURCES_PATH "hollowknight/") + std::string(assetsNames[i])).c_str()
				, requestedInfo);
		}



		sprites = enjinn::gl2d::loadTextureWithPixelPadding(ENJINN_RESOURCES_PATH "hollowknight/sprites.png", requestedInfo, 80);
...

		{
			...
			enjinn::sfs::safeLoad(data, ENJINN_RESOURCES_PATH "hollowknight/map", false);
			...
			{
				requestedInfo.log("Error reading map file", enjinn::logError);
			}
	...
	#pragma region input
		...
		::enjinn::gl2d::cameraController(renderer.currentCamera, input, 60 * (!followPlayer), 4);

		int direction = 0;
		

		if (input.buttons[enjinn::Button::Left].held() 
			|| input.anyController.buttons[enjinn::Controller::Left].held() 
			|| input.anyController.LStick.left()
			) { direction--; movingLeft = true; }

		if (input.buttons[enjinn::Button::Right].held() 
			|| input.anyController.buttons[enjinn::Controller::Right].held()
		...
		#pragma region jump
		{
			...
			if ((input.buttons[enjinn::Button::Space].pressed() ||
				input.anyController.buttons[enjinn::Controller::A].pressed())
				&& hitsGround
				)
			{
				duringJump = true;
			...
			if ((input.buttons[enjinn::Button::Space].held() ||
				input.anyController.buttons[enjinn::Controller::A].held()
				) 
					&& duringJump
				)
			{
	...
	#pragma region imgui
		{
			...
			if (ImGui::Button("Save map"))
			{
				...
				sfs::safeSave(data, ENJINN_RESOURCES_PATH "hollowknight/map", false);
		...
		requestedInfo.writeEntireFileBinary(ENJINN_RESOURCES_PATH "hollowknight/inputMetrics.bin",
			&inputMetrics, sizeof(inputMetrics));
