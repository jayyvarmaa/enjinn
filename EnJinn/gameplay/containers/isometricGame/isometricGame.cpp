...
bool IsometricGame::create(RequestedContainerInfo &requestedInfo, enjinn::StaticString<256> commandLineArgument)
{
	renderer.create(requestedInfo.requestedFBO.fbo);

	tiles = enjinn::gl2d::loadTextureWithPixelPadding(ENJINN_RESOURCES_PATH "iso/tiles/Isometric-Tiles.png", requestedInfo, 32, true);
	shadow = enjinn::gl2d::loadTexture(ENJINN_RESOURCES_PATH "iso/tiles/shadow.png", requestedInfo, 32, true);
	tilesAtlas = gl2d::TextureAtlasPadding(16, 8, tiles.GetSize().x, tiles.GetSize().y);

	playerSprite = enjinn::gl2d::loadTexture(ENJINN_RESOURCES_PATH "iso/tiles/IsoCharacter.png",
		requestedInfo, true);
	playerAtlas = gl2d::TextureAtlas(5, 4);

	itemsSprite = enjinn::gl2d::loadTexture(ENJINN_RESOURCES_PATH "iso/tiles/items.png",
		requestedInfo, true);
	itemsAtlas = gl2d::TextureAtlas(3, 1);

	itemFrameSprite = enjinn::gl2d::loadTexture(ENJINN_RESOURCES_PATH "iso/tiles/ui.png",
		requestedInfo, true);
	...
	for (int i = 0; i < MAPS_COUNT; i++)
	{
		...
		size_t s = 0;
		std::string fileName = ENJINN_RESOURCES_PATH "iso/maps/map" + std::to_string(i+1) + ".isomap";
	...
	size_t s = 0;
	if (requestedInfo.getFileSizeBinary(ENJINN_RESOURCES_PATH "iso/save.bin", s))
	{
		char *buffer = new char[s];
		
		if (requestedInfo.readEntireFileBinary(ENJINN_RESOURCES_PATH "iso/save.bin", buffer, s))
		{
...
bool IsometricGame::update(enjinn::Input input, enjinn::WindowState windowState, RequestedContainerInfo &requestedInfo)
{
...
void IsometricGame::saveData(RequestedContainerInfo &requestedInfo)
{
	...
	requestedInfo.writeEntireFileBinary(ENJINN_RESOURCES_PATH "iso/save.bin", fileData.data(), fileData.size());
