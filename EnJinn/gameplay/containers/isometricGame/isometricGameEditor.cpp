...
bool IsometricGameEditor::create(RequestedContainerInfo &requestedInfo, enjinn::StaticString<256> commandLineArgument)
{
	...
	loadedLevel.setInfo("Level", ENJINN_RESOURCES_PATH "iso/maps", {".isomap"});
	...
	if (commandLineArgument.size() > 0)
	{
		...
		size_t s = 0;
		enjinn::strlcpy(loadedLevel.file, commandLineArgument.to_string(), sizeof(loadedLevel.file));
...
bool IsometricGameEditor::update(enjinn::Input input, enjinn::WindowState windowState, RequestedContainerInfo &requestedInfo)
{
...
#pragma endregion
	...
	if (input.buttons[enjinn::Button::Z].pressed()) { currentBlock--; }
	if (input.buttons[enjinn::Button::X].pressed()) { currentBlock++; }
