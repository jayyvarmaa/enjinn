...
struct GameplayRenderer
{
	...
	bool init(RequestedContainerInfo &requestedInfo)
	{
		...
		size_t s = 0;
		if (requestedInfo.getFileSizeBinary(ENJINN_RESOURCES_PATH "/mario/1985_tiles.png", s))
		{
			void *data = new unsigned char[s];
			if (requestedInfo.readEntireFileBinary(ENJINN_RESOURCES_PATH "/mario/1985_tiles.png", data, s))
			{
		...
		marioTexture.loadFromFile(ENJINN_RESOURCES_PATH "/mario/mario.png", true, false);
	...
	void update(enjinn::Input &input, enjinn::WindowState &windowState,
	...
	void followPlayer(mario::Player &p, enjinn::Input &input, enjinn::WindowState &windowState)
	{
