...
struct MarioNeuralTrainer: public Container
{
	...
	enjinn::enjinnImgui::FileSelector currentMap;
	enjinn::enjinnImgui::FileSelector rezFile;
	enjinn::enjinnImgui::FileSelector graphFile;

	//todo user can request imgui ids; shortcut manager context; allocators
	static ContainerStaticInfo containerInfo()
	{
		ContainerStaticInfo info = {};
		info.defaultHeapMemorySize = enjinn::MB(400);
	...
	{
		currentMap.setInfo("Training map", ENJINN_RESOURCES_PATH "/mario", {".mario"});

		rezFile.setInfo("Neural network file", ENJINN_RESOURCES_PATH "/mario", {".neural"});
		graphFile.setInfo("Graph File", ENJINN_RESOURCES_PATH "/mario", {".txt"});


		if (commandLineArgument.size() != 0)
		{
			enjinn::strlcpy(currentMap.file, commandLineArgument.to_string(), sizeof(currentMap.file));
		}
		else
		{
			enjinn::strlcpy(currentMap.file, ENJINN_RESOURCES_PATH "/mario/map1.mario", sizeof(currentMap.file));
		}
