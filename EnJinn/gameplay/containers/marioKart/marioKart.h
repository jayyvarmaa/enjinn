...
struct MarioKart: public Container
{
	...
	bool create(RequestedContainerInfo &requestedInfo, enjinn::StaticString<256> commandLineArgument)
	{
		...
		//	false, 10);
		renderer.skyBox = renderer.loadSkyBox(ENJINN_RESOURCES_PATH "/skyBoxes/skybox.png");
		renderer.skyBox.color = glm::vec3(240.f / 255.f);

		editor.loadFromFile(renderer, ENJINN_RESOURCES_PATH "/marioKart/settings.gl3d", requestedInfo);
		...
		sphereModel = renderer.loadModel(ENJINN_RESOURCES_PATH "/marioKart/sphere.obj",
			gl3d::TextureLoadQuality::maxQuality, 1);
		...
		size_t size = 0;
		if (requestedInfo.getFileSizeBinary(ENJINN_RESOURCES_PATH "/marioKart/markers.bin", size))
		{
			...
			if (size % sizeof(MarioKartEditor::RoadMarker) == 0 && size)
			{
				...
				requestedInfo.readEntireFileBinary(ENJINN_RESOURCES_PATH "/marioKart/markers.bin",
					markers.data(), size);
			}
			else
			{
		...
		size = 0;
		if (requestedInfo.getFileSizeBinary(ENJINN_RESOURCES_PATH "/marioKart/coins.bin", size))
		{
			...
			if (size % sizeof(glm::vec3) == 0 && size)
			{
				...
				requestedInfo.readEntireFileBinary(ENJINN_RESOURCES_PATH "/marioKart/coins.bin",
					positions.data(), size);
	...
	#pragma region car
		...
		if (markers.size() >= 2)
		{
			...
			float moveForward = 0;
			if (input.buttons[enjinn::Button::Up].held() || (input.buttons[enjinn::Button::W].held() && !freeCamera))
			{
				moveForward = 1;
			}
			if (input.buttons[enjinn::Button::Down].held() || (input.buttons[enjinn::Button::S].held() && !freeCamera))
			{
				moveForward -= 1;
			}
			...
			if (acceleration)
			{
				...
				float leftRight = 0;
				if (input.buttons[enjinn::Button::Left].held() || (input.buttons[enjinn::Button::A].held() && !freeCamera))
				{
					leftRight = 1;
				}
				if (input.buttons[enjinn::Button::Right].held() || (input.buttons[enjinn::Button::D].held() && !freeCamera))
				{
					leftRight -= 1;
				}
		...
		if (input.buttons[enjinn::Button::P].pressed())
		{
			freeCamera = !freeCamera;
		}
		if (input.buttons[enjinn::Button::O].pressed())
		{
