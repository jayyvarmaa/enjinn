...
struct MarioKartEditor: public Container
{
	...
	#pragma region car
		...
		if (markers.size() >= 2)
		{
			...
			float moveForward = 0;
			if (input.buttons[enjinn::Button::Up].held())
			{
				moveForward = 1;
			}
			if (input.buttons[enjinn::Button::Down].held())
			{
				moveForward -= 1;
			}
	...
	#pragma endregion
		...
		if (ImGui::Begin("General3DEditor"))
		{
			if (ImGui::CollapsingHeader("Markers", ImGuiTreeNodeFlags_Framed | ImGuiTreeNodeFlags_FramePadding))
			{
				...
				{
					requestedInfo.writeEntireFileBinary(ENJINN_RESOURCES_PATH "/marioKart/markers.bin",
					...
					for (auto &c : coins)
					{
						...
						requestedInfo.writeEntireFileBinary(ENJINN_RESOURCES_PATH "/marioKart/coins.bin",
