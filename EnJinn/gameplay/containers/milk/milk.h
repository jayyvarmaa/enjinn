...
struct Milk: public Container
{
	...
	void load(RequestedContainerInfo &requestedInfo)
	{
		...
		requestedInfo.readEntireFileBinary(ENJINN_RESOURCES_PATH "milk/level.bin", binaryData);
	...
	bool create(RequestedContainerInfo &requestedInfo, enjinn::StaticString<256> commandLineArgument)
	{
		...
		renderer.skyBox = renderer.loadHDRSkyBox(ENJINN_RESOURCES_PATH "/skyBoxes/canary_wharf_2k.hdr");
		renderer.skyBox.color = {139.f / 255.f,139.f / 255.f,139.f / 255.f};
		floorMaterial = renderer.loadMaterial(ENJINN_RESOURCES_PATH "milk/floor/floor.mtl", gl3d::TextureLoadQuality::maxQuality)[0];
		...
		//helmetModel = renderer.loadModel(ENJINN_RESOURCES_PATH "helmet/helmet.obj", gl3d::TextureLoadQuality::maxQuality, 1.f);
		//helmetModel = renderer.loadModel(ENJINN_RESOURCES_PATH "/knight/uploads_files_1950170_Solus_the_knight.gltf", 1.f);


		//renderer.createDirectionalLight({-1,-0.5,-0.2});

		milkModel = renderer.loadModel(ENJINN_RESOURCES_PATH "milk/milk.glb", gl3d::TextureLoadQuality::maxQuality, 1);
		milkEntity = renderer.createEntity(milkModel, {{66.9,4.9,-9.5}}, false);

		gunModel = renderer.loadModel(ENJINN_RESOURCES_PATH "milk/bananagun.glb", gl3d::TextureLoadQuality::maxQuality, 0.08);
	...
	#pragma region frame start stuff
...

		{
			...
			if (input.buttons[enjinn::Button::Space].pressed() && playerObject.collidesBottom)
			{
	...
	#pragma endregion
		...
		if (ImGui::Begin("General3DEditor"))
		{
			...
			if (ImGui::Button("Save"))
			{
				...
				requestedInfo.writeEntireFileBinary(ENJINN_RESOURCES_PATH "milk/level.bin",
