...
struct McDungeonsGameplay: public Container
{
	...
	bool create(RequestedContainerInfo &requestedInfo, enjinn::StaticString<256> commandLineArgument)
	{
		...
		diamondTexture = enjinn::gl2d::loadTexture(ENJINN_RESOURCES_PATH "mcDungeons/diamond.png", requestedInfo, true, false);
		hearthTexture = enjinn::gl2d::loadTexture(ENJINN_RESOURCES_PATH "mcDungeons/hearth.png", requestedInfo, true, false);
		...
		renderer.colorCorrectionTexture() = renderer.loadColorLookupTextureFromFile(ENJINN_RESOURCES_PATH "/mcDungeons/lut.png");
		...
		//model = renderer.loadModel(ENJINN_RESOURCES_PATH "rave.glb", 0.5);
		auto defaultMat = renderer.loadMaterial(ENJINN_RESOURCES_PATH "materials/mcSprites/mc.mtl", gl3d::TextureLoadQuality::leastPossible); //todo quality settings
		...
		bool created = 0;
		size_t s = 0;
		if (requestedInfo.getFileSizeBinary(ENJINN_RESOURCES_PATH "/mcDungeons/map1.mcDungeons", s))
		{

			if (s == worldSize.x * worldSize.y * worldSize.z)
			{
				created = 1;
				requestedInfo.readEntireFileBinary(ENJINN_RESOURCES_PATH "/mcDungeons/map1.mcDungeons", worldData, sizeof(worldData));
			}
		}
		...
		diamondModel = renderer.loadModel(ENJINN_RESOURCES_PATH "mcDungeons/diamond.glb", 0, 1.f);
		

		auto zombieMat = renderer.loadMaterial(ENJINN_RESOURCES_PATH "mcDungeons/zombie.mtl", 0);
		if (zombieMat.size() != 1) { return false; }

		auto playerModel = renderer.loadModel(ENJINN_RESOURCES_PATH "mcDungeons/steve.glb", 0, 1);
		//auto mat = renderer.loadMaterial(ENJINN_RESOURCES_PATH "mcDungeons/steve.mtl", 0);
		...
		auto swordModel = renderer.loadModel(ENJINN_RESOURCES_PATH "/mcDungeons/minecraft_sword.glb", gl3d::TextureLoadQuality::maxQuality, 0.1);
	...
	#pragma region player
		...
		#pragma region input
			...
			if(!diamonds.empty())
			{
				...
				{
					if (input.buttons[enjinn::Button::A].held() || input.buttons[enjinn::Button::Left].held())
					{
						dir -= glm::vec2(1, 1);
					}
					if (input.buttons[enjinn::Button::D].held() || input.buttons[enjinn::Button::Right].held())
					{
						dir += glm::vec2(1, 1);
					}
					if (input.buttons[enjinn::Button::W].held() || input.buttons[enjinn::Button::Up].held())
					{
						dir += glm::vec2(1, -1);
					}
					if (input.buttons[enjinn::Button::S].held() || input.buttons[enjinn::Button::Down].held())
					{
						dir -= glm::vec2(1, -1);
					}
				}
				...
				if (input.buttons[enjinn::Button::Space].pressed() && attackCulldown <= 0)
				{
