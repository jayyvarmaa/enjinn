...
struct ThreeDGameExample: public Container
{
	...
	static ContainerStaticInfo containerInfo()
	{
		...
		info.defaultHeapMemorySize = enjinn::MB(1500); //todo option to use global allocator
	...
	enjinn::GL::EnJinnFramebuffer frameBuffers[2];
	...
	gl3d::Model createPlane(gl3d::Renderer3D &renderer)
	{
		...
		auto material =  renderer.loadMaterial(ENJINN_RESOURCES_PATH "materials/rustedIron.mtl", gl3d::maxQuality);
	...
	bool create(RequestedContainerInfo &requestedInfo, enjinn::StaticString<256> commandLineArgument)
	{
		...
		//font = enjinn::gl2d::loadFont(ENJINN_RESOURCES_PATH "arial.ttf", requestedInfo);
		...
		//enjinn::gl3d::loadSettingsFromFileName(renderer, ENJINN_RESOURCES_PATH "/threedgame/settings.gl3d", requestedInfo);
		editor.loadFromFile(renderer, ENJINN_RESOURCES_PATH "/threedgame/settings.gl3d", requestedInfo);
		//renderer.skyBox.color = glm::vec3(0.8f);


		zombieMat = renderer.loadMaterial(ENJINN_RESOURCES_PATH "threedgame/zombie.mtl", 0);
		if (zombieMat.size() != 1) { return false; }


		playerM = renderer.loadModel(ENJINN_RESOURCES_PATH "mcDungeons/steve.glb", 0, 1);
...

		{
			...
			requestedInfo.readEntireFileBinary(ENJINN_RESOURCES_PATH "threedgame/ui.sushi", loader.data);
	...
	#pragma endregion
		...
		if (!player2joined && input.buttons[enjinn::Button::Enter].pressed())
		{
			player2joined = true;
		...
		if (input.buttons[enjinn::Button::F].released())
		{
			...
			
			{
				if (!freeCamera)
				{
					...
					auto updatePlayer = [&](PlayerModel &p, bool differentInput = 0)
					{
						...
						{
							if (
								(!differentInput && input.buttons[enjinn::Button::A].held()) ||
								(differentInput && input.buttons[enjinn::Button::Left].held()) )
							{
								dir -= glm::vec2(1, 1);
							}
							if (
								(!differentInput && input.buttons[enjinn::Button::D].held() )								
								|| (differentInput && input.buttons[enjinn::Button::Right].held()) )
							{
								dir += glm::vec2(1, 1);
							}
							if (
							  	(!differentInput && input.buttons[enjinn::Button::W].held() )
								|| (differentInput && input.buttons[enjinn::Button::Up].held()))
							{
								dir += glm::vec2(1, -1);
							}
							if ((!differentInput && input.buttons[enjinn::Button::S].held() )
								|| (differentInput && input.buttons[enjinn::Button::Down].held()))
							{
								dir -= glm::vec2(1, -1);
							}
						}
						...
						if ((!differentInput && input.buttons[enjinn::Button::Space].pressed()) ||
							(differentInput && input.buttons[enjinn::Button::Enter].pressed())
							)
						{
