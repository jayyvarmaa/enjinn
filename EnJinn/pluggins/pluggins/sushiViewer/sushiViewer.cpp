...
ContainerStaticInfo SushiViewer::containerInfo()
{
	...
	info.defaultHeapMemorySize = enjinn::MB(50);
...
bool SushiViewer::create(RequestedContainerInfo &requestedInfo, enjinn::StaticString<256> commandLineArgument)
{
...
void SushiViewer::displaySushiBackgroundImgui(::sushi::Background &e, int id)
{
	if (ImGui::BeginChildFrame(id, {0, 100}, true))
	{
		...
		enjinn::enjinnImgui::ColorEdit4Swatches("Background color", &e.color[0]);
...
void SushiViewer::displaySushiTextElementImgui(::sushi::Text &e, glm::vec4 parent, int id)
{
	...
	::enjinn::enjinnImgui::ColorEdit4Swatches("Text color: ", &e.color[0]);
	...
	bool displayChildren)
{
	...
	if (enjinn::enjinnImgui::BeginChildFrameColoured(e.id, color, { 0, 700 }, true))
	{
		...
		{
			if (enjinn::enjinnImgui::blueButton("Select"))
			{
				img.elementId = e.id;
			}
		}
		...
		if (displayChildren)
		{
			...
			if (enjinn::enjinnImgui::greenButton("Add parent"))
			{
...
bool SushiViewer::update(enjinn::Input input, enjinn::WindowState windowState, RequestedContainerInfo &requestedInfo)
{
	bool leftCtrlHeld = input.buttons[enjinn::Button::LeftCtrl].held();
	//bool shiftHeld = input.buttons[enjinn::Button::Shift].held();
...
#pragma region update stuff
	...
	//else
	//{
	//	sushiContext.update(renderer, enjinn::toSushi(input));
	//}
#pragma endregion
	...
	#pragma endregion
		...
		if (selectedParent)
		{
			...
			if (parentOfParent)
			{
				...
				if (::enjinn::enjinnImgui::blueButton("Select parent"))
				{
